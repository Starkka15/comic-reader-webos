...
static void render_reader(UIState *ui) {
    ...
    if (page) {
        if (ui->zoom <= 1.01f) {
            // Normal view - center the page
            int x = (SCREEN_WIDTH - page->w) / 2;
            int y = (SCREEN_HEIGHT - 40 - page->h) / 2;  // Account for status bar
            SDL_Rect dest = {x, y, 0, 0};
    ...
    char page_info[64];
    if (ui->zoom > 1.01f) {
        snprintf(page_info, sizeof(page_info), "Page %d / %d  [%.0fx]",
    ...
    // Navigation hint
    draw_text(ui->screen, ui->font_small, "Swipe edge: page | Tap: zoom", 350, SCREEN_HEIGHT - 30, COLOR_GRAY);
...
int ui_handle_event(UIState *ui, SDL_Event *event) {
    ...
    if (event->type == SDL_MOUSEBUTTONUP) {
        ...
        if (ui->state == SCREEN_BROWSER && !ui->touch_moved) {
            ...
            int started_at_right_edge = (start_x > SCREEN_WIDTH - edge_zone);
